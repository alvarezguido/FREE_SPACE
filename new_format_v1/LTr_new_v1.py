...
def simulate_scenario (nrNodes):
    ...
    def selectSF (env, node):
        ...
        if rssi > sf7[1]:
            #print ("----Select SF7")
            #node.packet.sf = 7
            node.packet.sf = random.choice([7,8,9])
            c7+=1
        elif rssi > sf8[1]:
            #print ("----Select SF8")
            #node.packet.sf = 8
            node.packet.sf = random.choice([8,9,10])
            c8+=1
        elif rssi > sf9[1]:
            #print ("----Select SF9")
            #node.packet.sf = 9
            node.packet.sf = random.choice([9,10,11])
            c9+=1
        elif rssi > sf10[1]:
            #print ("----Select SF10")
            #node.packet.sf = 10
...
#     fname = "./LTr_1CH/" + str("LTr_"+str(nrNodes)+"_1CH_"+str(maxBSReceives)) + ".csv"
...
for nrNodes in multi_nodes:
    ...
    fname = "./LTr_3CH/" + str("LTr_"+str(nrNodes)+"_3CH_"+str(maxBSReceives)) + ".csv"
    with open(fname,"w") as myfile:
        myfile.write("\n".join(logs))
    myfile.close()
    i=i+1
...
if plots_nodes == 1:
    ...
    plt.subplot(1,3,2)
    plt.title("Distance from node %i to Leo Sat [BEST NODE]"%node_best)
    plt.xlabel("Time/step Leo Sat passing by")
    plt.ylabel("Distance in [km]")
    plt.grid()
    plt.plot(distance[node_best,:])

    plt.subplot(1,3,3)
    plt.title("Distance from node %i to Leo Sat [WORST NODE]"%node_worst)
    plt.xlabel("Time/step Leo Sat passing by")
    plt.ylabel("Distance in [km]")
    plt.grid()
    plt.plot(distance[node_worst,:])
    plt.show()