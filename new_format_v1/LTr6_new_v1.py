...
def simulate_scenario (nrNodes):
    ...
    def selectSF (env, node):
        ...
        if rssi > sf7[1]:
            #print ("----Select SF7")
            #node.packet.sf = 7
            node.packet.sf = random.choice([7,8,9,10,11,12])
            c7+=1
        elif rssi > sf8[1]:
            #print ("----Select SF8")
            #node.packet.sf = 8
...
#     fname = "./LTr6_1CH/" + str("LTr6_"+str(nrNodes)+"_1CH_"+str(maxBSReceives)) + ".csv"
...
for nrNodes in multi_nodes:
    ...
    fname = "./LTr6_3CH/" + str("LTr6_"+str(nrNodes)+"_3CH_"+str(maxBSReceives)) + ".csv"
    with open(fname,"w") as myfile:
        myfile.write("\n".join(logs))
    myfile.close()
    i=i+1
