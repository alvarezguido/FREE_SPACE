...
def simulate_scenario (nrNodes):
    ...
    def selectSF (env, node):
        ...
        if rssi > sf7[1]:
            #print ("----Select SF7")
            #node.packet.sf = 7
            node.packet.sf = random.choice([7,8,9,10])
            c7+=1
        elif rssi > sf8[1]:
            #print ("----Select SF8")
            #node.packet.sf = 8
            node.packet.sf = random.choice([8,9,10,11])
            c8+=1
        elif rssi > sf9[1]:
            #print ("----Select SF9")
            #node.packet.sf = 9
...
#     fname = "./LTr4_1CH/" + str("LTr4_"+str(nrNodes)+"_1CH_"+str(maxBSReceives)) + ".csv"
...
for nrNodes in multi_nodes:
    ...
    fname = "./LTr4_3CH/" + str("LTr4_"+str(nrNodes)+"_3CH_"+str(maxBSReceives)) + ".csv"
    with open(fname,"w") as myfile:
        myfile.write("\n".join(logs))
    myfile.close()
    i=i+1
