...
multi_nodes = [1000,2000,3000,4000,5000,6000,7000,8000,9000,10000,11000,12000,13000,14000]
...
##UPDATED VALUES FOR SENSIBILITY LoRa-E
## [DR8,DR9,DR10,DR11]
sensi_E = np.array([-134,-131.5,-131,-128.5])
            ...
            if node in packetsAtBS:
                ...
                if node.packet.rssi[math.ceil(env.now)] < sensibility: #HERE WE ARE CONSIDERING RSSI AT TIME ENV.NOW
                    ...
                    if node in packetsAtBS:
                        ...
                        sensibility = sensi[node.packet.sf - 7, [125,250,500].index(node.packet.bw) + 1]
                        node.dr = selectDR(env,node)
                        
                        if node.dr == "dr8":
                            sensibility = sensi_E[0]
                        elif node.dr == "dr9":
                            sensibility = sensi_E[1]
                        elif node.dr == "dr10":
                            sensibility = sensi_E[2]
                        else:
                            sensibility = sensi_E[3]
...
for nrNodes in multi_nodes:
    ...
    fname = "./ET_1CH_14k/" + str("ET_"+str(nrNodes)+"_1CH_"+str(maxBSReceives)) + ".csv"
...
#     fname = "./ET_3CH_14k/" + str("ET_"+str(nrNodes)+"_3CH_"+str(maxBSReceives)) + ".csv"
#     with open(fname,"w") as myfile:
#         myfile.write("\n".join(logs))
#     myfile.close()
#     i=i+1
